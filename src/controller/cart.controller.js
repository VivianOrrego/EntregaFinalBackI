...
export const create = async (req, res, next) => {
    try {
        ...
        if (!newCart) res.status(404).json({
            msg: "Error create cart!"
        });
...
export const update = async (req, res, next) => {
    try {
        ...
        if (!cartUpd) res.status(404).json({
            msg: "Error update cart!"
        });
...
export const remove = async (req, res, next) => {
    try {
        ...
        if (!cartDel) res.status(404).json({
            msg: "Error delete cart!"
        });
        else res.status(200).json({
            msg: `Cart id: ${id} deleted`
        });
    } catch (error) {
        next(error.message);
    }
...
export const addProdToCart = async (req, res, next) => {
    try {
        ...
        if (!newProdToUserCart) res.json({
            msg: "Error add product to cart"
        });
...
export const removeProdToCart = async (req, res, next) => {
    try {
        ...
        if (!delProdToUserCart) res.json({
            msg: "Error remove product to cart"
        });
        else res.json({
            msg: `product ${idProd} deleted to cart`
        });
    } catch (error) {
        next(error.message);
    }
...
export const updateProdQuantityToCart = async (req, res, next) => {
    try {
        ...
        if (!updateProdQuantity) res.json({
...
export const clearCart = async (req, res, next) => {
    try {
        ...
        if (!clearCart) res.json({
            msg: "Error clear cart"
        });
